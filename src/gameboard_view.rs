...
/// This is responseable for all rendering and stuff...
...
impl GameboardViewSettings {
    ...
    pub fn new() -> GameboardViewSettings {
        GameboardViewSettings {
            position: [10.0; 2],
            size: 400.0,
            background_color: [0.9, 0.9, 1.0, 1.0],
            ...
            selected_cell_background_color: [0.8, 0.8, 1.0, 1.0],
...
impl GameboardView {
        ...
        C: CharacterCache<Texture = G::Texture>,
    {
        ...
        // draw selected Cell
        if let Some(cell) = controller.selected_cell {
            let cell_size = settings.size / 9.0;
            let pos = [
                cell[0] as f64 * cell_size + settings.position[0],
                cell[1] as f64 * cell_size + settings.position[1],
            ];
        ...
        for j in 0..9 {
            for i in 0..9 {
                if let Some(ch) = controller.game_state.gameboard.char([i, j]) {
                    if ch == '0' {
                        continue;
                    }
                    let pos = [
